...
public class AsconMain {
	public final static int MAXLEN = 65536;
	  ...
	  public static void main(String[] args) {
	    ...
	    int alen = MAXLEN;
	    int mlen = MAXLEN;
	    int clen = MAXLEN + Ascon.CRYPTO_ABYTES;
	    ...
	    for (alen = 0; alen <= MLEN; ++alen)
	      for (mlen = 0; mlen <= MLEN; ++mlen) {
	        Ascon.print("k", k, Ascon.CRYPTO_KEYBYTES);
	        System.out.printf("\n");
	        Ascon.print("n", npub, Ascon.CRYPTO_NPUBBYTES);
	        System.out.printf("\n");
	        Ascon.print("a", a, alen);
	        System.out.printf("\n");
	        Ascon.print("m", m, mlen);
	        System.out.printf("\n");
	        clen = Ascon.crypto_aead_encrypt(c, clen, m, mlen, a, alen, nsec, npub, k);
	        Ascon.print("c", c, clen - Ascon.CRYPTO_ABYTES);
	        System.out.printf("\n");
	        Ascon.print("t", new String(c).substring((int) (clen - Ascon.CRYPTO_ABYTES))
	            .getBytes(), Ascon.CRYPTO_ABYTES);
	        System.out.printf("\n");
	        mlen = Ascon.crypto_aead_decrypt(m, mlen, nsec, c, clen, a, alen, npub, k);
	        mlen = Ascon.crypto_aead_decrypt(m, mlen, nsec, c, clen, a, alen, npub, k);
	        if (mlen != -1) {
	          Ascon.print("p", m, mlen);
	          System.out.printf("\n");
	        } else
